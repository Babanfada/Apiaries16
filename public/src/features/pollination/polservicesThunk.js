...
export const useCreatePolServices = () => {
  ...
  const { mutate: createPolService, status: isCreatingPolservice } =
    useMutation({
...
export const useUpdatePolServices = () => {
  const dispatch = useDispatch();
  const queryClient = useQueryClient();
  const { mutate: updatePolService, status: isUpdatingPolservices } =
    useMutation({
...
export const useDeletePolServices = () => {
  const queryClient = useQueryClient();
  const { mutate: deletePolService, status: isDeletingPolServices } =
    useMutation({
      mutationFn: async (id) => {

...
export const useConsultation = () => {
  const { pages, numOfTimesRendered, sort, item_name, priceRangeC } =
    useSelector((store) => store.consultations);
  const numberFilterParams = [
    numOfTimesRendered !== undefined
      ? `numOfTimesRendered<=${numOfTimesRendered}`
      : "",
...
export const useCreateConsultation = () => {
  ...
  const { mutate: createConsultation, status: isCreatingConsultation } =
    useMutation({
...
export const useUpdateConsultation = () => {
  const dispatch = useDispatch();
  const queryClient = useQueryClient();
  const { mutate: updateConsultation, status: isUpdatingConsultation } =
    useMutation({
...
export const useDeleteConsultation = () => {
  const queryClient = useQueryClient();
  const { mutate: deleteConsultation, status: isDeletingConsultation } =
    useMutation({
      mutationFn: async (id) => {
